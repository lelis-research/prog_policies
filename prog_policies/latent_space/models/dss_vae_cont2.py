...
class Accuracies(NamedTuple):
    ...
    adv_struct_t: torch.Tensor
    adv_struct_s: torch.Tensor
...
class DSSVAECont2(BaseVAE):
    ...
    def __init__(self, dsl: BaseDSL, device: torch.device, env_cls: type[BaseEnvironment],
        ...
        syn_disc_gru = nn.GRU(self.syn_latent_size + self.num_program_tokens, self.hidden_size)
        init_gru(syn_disc_gru)
        
        syn_disc_mlp = nn.Sequential(
            self.init_(nn.Linear(self.hidden_size + self.num_program_tokens + self.syn_latent_size,
                                 self.hidden_size)),
            nn.Tanh(), self.init_(nn.Linear(self.hidden_size, self.num_program_tokens))
        )
        
        self.syn_disc = ProgramSequenceDecoder(token_encoder, syn_disc_gru,
                                               syn_disc_mlp, self.device, dsl,
                                               max_program_length, only_structure=False)
        
        sem_disc_gru = nn.GRU(self.sem_latent_size + self.num_program_tokens, self.hidden_size)
        init_gru(sem_disc_gru)
        
        sem_disc_mlp = nn.Sequential(
            self.init_(nn.Linear(self.hidden_size + self.num_program_tokens + self.sem_latent_size,
                                 self.hidden_size)),
            nn.Tanh(), self.init_(nn.Linear(self.hidden_size, self.num_program_tokens))
        )
        
        self.sem_disc = ProgramSequenceDecoder(token_encoder, sem_disc_gru,
                                              sem_disc_mlp, self.device, dsl,
                                              max_program_length, only_structure=False)
    ...
    def get_losses_and_accs(self, data_batch: tuple, output: tuple,
        ...
        progs_loss, rec_a_h_loss, rec_struct_loss, disc_a_h_loss, disc_struct_loss, \
            sem_disc_loss, syn_disc_loss, latent_loss = zero_tensor, zero_tensor, zero_tensor, \
                zero_tensor, zero_tensor, zero_tensor, zero_tensor, zero_tensor
        ...
        with torch.no_grad():
            ...
            if disc_a_h is not None:
                disc_a_h_masks_combined = torch.max(a_h_masks, disc_a_h_masks)
                adv_a_h_t_accuracy = (disc_a_h[disc_a_h_masks_combined] == a_h[disc_a_h_masks_combined]).float().mean()
                adv_a_h_s_accuracy = (a_h == disc_a_h).min(dim=1).values.float().mean()
            
            adv_struct_s_accuracy, adv_struct_t_accuracy = zero_tensor, zero_tensor
            if disc_struct is not None:
                disc_struct_masks_combined = torch.max(progs_masks, disc_struct_masks)
                adv_struct_t_accuracy = (disc_struct[disc_struct_masks_combined] == structs[disc_struct_masks_combined]).float().mean()
                adv_struct_s_accuracy = (structs == disc_struct).min(dim=1).values.float().mean()
        
        return Losses(progs_loss, rec_a_h_loss, disc_a_h_loss, sem_disc_loss,
                      rec_struct_loss, disc_struct_loss, syn_disc_loss, latent_loss),\
            Accuracies(progs_t_accuracy, progs_s_accuracy, a_h_t_accuracy, a_h_s_accuracy,
                       struct_t_accuracy, struct_s_accuracy, adv_a_h_t_accuracy, adv_a_h_s_accuracy,
                       adv_struct_t_accuracy, adv_struct_s_accuracy)
    ...
    def fit(self, train_dataloader: DataLoader, val_dataloader: DataLoader,
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for i, train_batch in enumerate(train_dataloader):
                ...
                total_adv_loss = adv_losses.a_h_disc + adv_losses.struct_disc + adv_losses.syn_disc + adv_losses.sem_disc
