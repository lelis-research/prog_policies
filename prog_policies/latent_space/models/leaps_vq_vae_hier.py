...
class LeapsVQVAE_Hier(BaseVAE):
    def __init__(self, dsl: BaseDSL, device: torch.device, env_cls: type[BaseEnvironment],
        ...
        self.prog_decoder = ProgramSequenceDecoder(
            ...
            mlp=nn.Sequential(
                self.init_(nn.Linear(2 * self.hidden_size + self.num_program_tokens,
                                    self.hidden_size)),
        ...
        self.downsamplers = [
            nn.Sequential(
                self.init_(nn.Linear(self.hidden_size // (4 ** i), self.hidden_size // (4 ** i))),
                nn.Tanh(),
                self.init_(nn.Linear(self.hidden_size // (4 ** i), self.hidden_size // (4 ** (i+1))))
            ) for i in range(n_vq-1)
        ]
        
        self.upsamplers = [
            nn.Sequential(
                self.init_(nn.Linear(self.hidden_size // (4 ** (i+1)), self.hidden_size // (4 ** (i+1)))),
                nn.Tanh(),
                self.init_(nn.Linear(self.hidden_size // (4 ** (i+1)), self.hidden_size // (4 ** i)))
            ) for i in range(n_vq-1)
        ]
        
        # Encoder VAE utils
        self.quantizers = [
            VectorQuantizer(self.device, vq_size, self.hidden_size // (4 ** i), 0.25)
            for i in range(n_vq)
        ]
        
        self.to(self.device)

    def sample_latent_vector(self, vq_inputs: list[torch.Tensor]) -> list[torch.Tensor]:
        quantized_states = []
        losses = []
        for vq_input, quantizer in zip(vq_inputs, self.quantizers):
            quantized_state = quantizer(vq_input.squeeze(-1))
            quantized_states.append(quantized_state)
            losses.append(quantizer.get_loss())
    ...
    def forward(self, data_batch: tuple, prog_teacher_enforcing = True,
        ...
        vq_inputs = [enc_hidden_state]
        for ds in self.downsamplers:
            vq_inputs.append(ds(vq_inputs[-1]))
        
        vq_outputs = self.sample_latent_vector(vq_inputs)
        
        z = vq_outputs[-1]
        for us, vq_output in zip(reversed(self.upsamplers), reversed(vq_outputs[:-1])):
            z = us(z) + vq_output
